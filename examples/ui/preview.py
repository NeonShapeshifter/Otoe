...
def build_preview_html() -> str:
    app = mount(
        UIKitKitchenSink(
            ...
            palette_open=signal(True),
            menu_open=signal(True),
            menu_action=signal("inspect"),
            menu_focus=signal("inspect"),
            density=signal("balanced"),
            density_open=signal(True),
            active_route=signal("ui"),
            on_query=lambda value: None,
            on_select=lambda command_id: None,
            on_open_palette=lambda: None,
            on_toggle_dialog=lambda: None,
            on_toggle_menu=lambda: None,
            on_menu_select=lambda item_id: None,
            on_menu_focus=lambda item_id: None,
            on_menu_open_change=lambda value: None,
            on_density_change=lambda value: None,
            on_density_open_change=lambda value: None,
